...
from kanbansim.models import Run, Case, Workflow, Work, StoreWithHistory, FilterStoreWithHistory
from kanbansim.agents import BA, QA, Releaser, Developer
...
def monitor(env, finisher, progress_pile, total_values, work_piles, done_pile, released_pile, standard_cases):
    while True:
        if len(released_pile.items) == len(standard_cases) or env.now > SIM_TIME:
        ...
        for pile in work_piles.values():
            pile.update_history()

        done_pile.update_history()
        released_pile.update_history()

        yield env.timeout(1)


def sourcer(name, env, _source_pile, _dispatch_pile, _progress_pile, _work_piles):
    """
    ...
    while True:
        ...
        if len(_work_piles[wt.WORK_ANALYSIS].items) <= ANALYSIS_PILE_MIN:
            logging.debug('%s fetching a source case at %s' % (name, env.now))
            case = yield _source_pile.get()
            ...
            dispatch_new_case_work(case, _dispatch_pile, _progress_pile, _work_piles)
            logging.info('%s dispatching %s at %s' % (name, case, env.now))
        else:
            yield env.timeout(1)


def dispatch_new_case_work(case, _dispatch_pile, _progress_pile, _work_piles):
    """
    Initialises work queues with work items for a new case
    :param case:
    :param _dispatch_pile:
    :param _progress_pile:
    :param _work_piles:
    :return:
    """
    # Track the case on the progress pile
    _progress_pile.put(case)

    # Add the cases first work items to the dispatch pile
    items = case.workflow.get_next_work_items()
    if items is not None:
        for item in items:
            dispatch_work_item(item, _dispatch_pile, _work_piles)


def dispatch_work_item(item, dispatch_pile, work_piles):
    """
    ...
    if item is not None:
        if issubclass(type(item), Workflow):
            ...
            assign_work_to_pile(item, work_piles)
        else:
            raise Exception("Incorrect work item type: %s" % type(item))


def dispatcher(name, env, dispatch_pile, work_piles):
    """
    ...
    while True:
        ...
        for item in dispatch_pile:
            dispatch_work_item(item, dispatch_pile, work_piles)
...
def case_done_listener(done_pile=None, case=None):
    # Move the case to the done_pile pile
    assert done_pile is not None, "case_done_listener: No done pile provided"
    assert case is not None, "case_done_listener: No case provided"
    done_pile.put(case)


def dispatch_work_listener(dispatch_pile=None, work=None):
    # Move the case to the done_pile pile
    assert dispatch_pile is not None, "case_done_listener: No dispatch pile provided"
    assert work is not None, "dispatch_work_listener: No work provided"
    dispatch_pile.append(work)


def dispatch_next_work_items_listener(dispatch_pile=None, work_piles=None, workflow=None):
    """
    Get the next work items for the specified workflow and dispatch them
    :param dispatch_pile:
    :param work_piles:
    :param workflow:
    :return:
    """
    items = workflow.get_next_work_items()
    if items is not None:
        for item in items:
            dispatch_work_item(item, dispatch_pile, work_piles)


def assign_work_to_pile(work, _work_piles):
    """
    ...
    :return:
    """
    if work.name in _work_piles.keys():
        logging.debug('Assigning work item %s to pile %s at %s' % (work, work.name, work.env.now))
        _work_piles[work.name].put(work)
...
def main():
    ...
    for run in tqdm(range(NUM_RUNS)):
        ...
        for case in standard_cases[:NUM_INITIAL_CASES]:
            _case = copy.deepcopy(case)
            _case.set_env(env)
            dispatch_new_case_work(_case, dispatch_pile, progress_pile, work_piles)
        ...
        # Create a Dispatcher
        dispatchers = [env.process(dispatcher('Dispatcher 0', env, dispatch_pile, work_piles))]

        # Create a case Sourcer
        sourcers = [env.process(sourcer('Sourcer 0', env, source_pile, dispatch_pile, progress_pile, work_piles))]

        # add monitoring, which will also terminate the sim on completion of all work
        finisher = simpy.Event(env)
        env.process(monitor(env, finisher, progress_pile, total_values, work_piles, done_pile, released_pile,
                            standard_cases))

        pub.subscribe(dispatch_work_listener, 'dispatch_work', dispatch_pile=dispatch_pile)
        pub.subscribe(case_done_listener, 'case_is_done', done_pile=done_pile)
        pub.subscribe(dispatch_next_work_items_listener, 'dispatch_next_work_items', dispatch_pile=dispatch_pile,
    ...
    for sim_run in runs:
        ...
        _ = plt.plot(sim_run.run_data('total_value'), alpha=0.1, color=color, linestyle=line_style)
    plt.show()
